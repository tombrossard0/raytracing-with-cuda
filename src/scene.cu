...
void Scene::processKeyboard(const Uint8 *keystate, float deltaTime)
{
    ...
    if (keystate[SDL_SCANCODE_W])
        center = center - forward * speed; // forward
    if (keystate[SDL_SCANCODE_S])
        center = center + forward * speed; // backward
    if (keystate[SDL_SCANCODE_A])
        center = center + right * speed; // left
    if (keystate[SDL_SCANCODE_D])
        center = center - right * speed; // right
    if (keystate[SDL_SCANCODE_SPACE])
        center = center - up * speed; // up
    if (keystate[SDL_SCANCODE_LCTRL])
        center = center + up * speed; // down
}
